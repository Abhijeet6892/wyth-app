...
function ChatContent() {
  ...
  // Partner deleted notice
  const [partnerDeleted, setPartnerDeleted] = useState(false);
  const [noticeExpiresAt, setNoticeExpiresAt] = useState<string | null>(null);
    ...
    const loadData = async () => {
      ...
      if (user) {
        ...
        const { data } = await supabase
          .from("connections")
          .select(`*, partner_notice_expires_at, profiles:receiver_id (id, full_name, avatar_url, is_gold, profile_signals, city, intent, is_deleted)`)
  ...
  useEffect(() => {
    ...
    // Check if partner has been soft deleted
    const isDeleted = activeChat.profiles?.is_deleted === true;
    const expiresAt = activeChat.partner_notice_expires_at ?? null;
    setPartnerDeleted(isDeleted);
    setNoticeExpiresAt(expiresAt);
  ...
  return (
    <div style={{
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
              <button
                ...
                style={{
                  ...
                  border: 'none', color: '#1E3A8A', cursor: 'pointer',
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                <div style={{
                  ...
                  background: activeChat.profiles.is_gold 
                    ? 'linear-gradient(135deg, #fde68a 0%, #fbbf24 100%)' : '#f1f5f9',
                  padding: '2px'
                }}>
                  <div style={{
                ...
                <div>
                  ...
                  <div style={{ fontSize: '12px', color: partnerDeleted ? '#ef4444' : '#94a3b8' }}>
                    {partnerDeleted ? 'No longer on Wyth' : activeChat.profiles.city}
                  </div>
                </div>
              </div>
            </div>
            ...
            <div style={{ position: 'relative' }}>
              ...
              {showOptionsMenu && (
                <div style={{
                  ...
                  borderRadius: '16px', boxShadow: '0 8px 32px rgba(31, 41, 55, 0.15)',
                  padding: '8px', minWidth: '200px', zIndex: 50
                }}>
                  <button
                    onClick={() => { router.push(`/profile/${activeChat.profiles.full_name}`); setShowOptionsMenu(false); }}
                    style={{
                      width: '100%', padding: '12px 16px', background: 'transparent',
                      border: 'none', borderRadius: '12px', textAlign: 'left', cursor: 'pointer',
                      transition: 'background 0.2s', display: 'flex', alignItems: 'center',
                      gap: '12px', color: '#1e3a8a', fontSize: '14px', fontWeight: '500'
                    }}
                  ...
                  <button
                    onClick={() => { handleDisconnect(); setShowOptionsMenu(false); }}
                    style={{
                      width: '100%', padding: '12px 16px', background: 'transparent',
                      border: 'none', borderRadius: '12px', textAlign: 'left', cursor: 'pointer',
                      transition: 'background 0.2s', display: 'flex', alignItems: 'center',
                      gap: '12px', color: '#64748b', fontSize: '14px', fontWeight: '500'
                    }}
                  ...
                  <button
                    onClick={() => { handleBlock(); setShowOptionsMenu(false); }}
                    style={{
                      width: '100%', padding: '12px 16px', background: 'transparent',
                      border: 'none', borderRadius: '12px', textAlign: 'left', cursor: 'pointer',
                      transition: 'background 0.2s', display: 'flex', alignItems: 'center',
                      gap: '12px', color: '#ef4444', fontSize: '14px', fontWeight: '500'
                    }}
      ...
      <div style={{ 
        ...
        {/* SIDEBAR */}
        <div style={{
          width: activeChat ? '0' : '100%', maxWidth: '400px',
          overflow: 'hidden', transition: 'width 0.3s ease',
          padding: activeChat ? '0' : '20px', margin: '0 auto'
        }}>
          {!activeChat && connections.length === 0 && (
            <div style={{
              ...
              backdropFilter: 'blur(20px)', WebkitBackdropFilter: 'blur(20px)',
              border: '1px solid rgba(255, 255, 255, 0.5)',
              borderRadius: '24px', boxShadow: '0 8px 32px rgba(31, 41, 55, 0.1)'
            }}>
              ...
              <Link
                ...
                style={{
                  ...
                  boxShadow: '0 4px 16px rgba(30, 58, 138, 0.3)', transition: 'all 0.2s'
                }}
                onMouseEnter={(e) => {
          ...
          {!activeChat && connections.map((conn) => (
            <div
              ...
              style={{
                ...
                backdropFilter: 'blur(20px)', WebkitBackdropFilter: 'blur(20px)',
                border: '1px solid rgba(255, 255, 255, 0.5)',
                borderRadius: '20px', boxShadow: '0 4px 16px rgba(31, 41, 55, 0.08)',
            ...
            >
              ...
              <div style={{ flex: 1 }}>
                ...
                <div style={{ fontSize: '12px', color: conn.profiles.is_deleted ? '#ef4444' : '#94a3b8' }}>
                  {conn.profiles.is_deleted ? 'No longer on Wyth' : `${conn.profiles.city} • ${conn.profiles.intent}`}
                </div>
              </div>
            </div>
          ))}
        ...
        {activeChat && (
          <div style={{
            ...
            {/* Partner deleted notice banner */}
            {partnerDeleted && noticeExpiresAt && new Date(noticeExpiresAt) > new Date() && (
              <div style={{
                ...
                <p style={{ fontSize: '14px', fontWeight: '600', color: '#475569', margin: '0 0 4px' }}>
                  This person is no longer on Wyth
                </p>
                <p style={{ fontSize: '12px', color: '#94a3b8', margin: 0, lineHeight: '1.5' }}>
                  This conversation will be archived soon. You cannot send new messages.
                </p>
              </div>
            )}
            ...
            {messages.map((m, idx) => {
              ...
              return (
                <div
                  key={idx}
                  style={{ display: 'flex', justifyContent: isMe ? 'flex-end' : 'flex-start', marginBottom: '12px' }}
                >
                  <div style={{
                    maxWidth: '70%', padding: '12px 16px', borderRadius: '16px', fontSize: '14px',
                    background: isMe 
                    ...
                    boxShadow: isMe ? '0 4px 12px rgba(30, 58, 138, 0.2)' : '0 2px 8px rgba(31, 41, 55, 0.05)',
            ...
            {isLimitReached && (
              <div style={{ display: 'flex', justifyContent: 'center', margin: '24px 0' }}>
                <span style={{
                  ...
                  background: '#fef2f2', padding: '8px 16px', borderRadius: '12px',
                  border: '1px solid #fecaca', display: 'flex', alignItems: 'center',
                  gap: '8px', boxShadow: '0 2px 8px rgba(239, 68, 68, 0.1)'
                }}>
      ...
      {activeChat && (
        <div style={{
          ...
          padding: '12px', zIndex: 60, paddingBottom: '24px'
        }}>

          {/* AI Menu */}
          {showAiMenu && !partnerDeleted && (
            <div style={{
              ...
              borderRadius: '20px', padding: '16px', width: '280px', zIndex: 20
            }}>
              <div style={{
                ...
                <span style={{
                  ...
                  textTransform: 'uppercase', display: 'flex', gap: '4px',
                  alignItems: 'center', letterSpacing: '0.5px'
                }}>
              ...
              <div style={{
                ...
                padding: '4px', gap: '4px', marginBottom: '12px'
              }}>
                {(["Grounded", "Thoughtful", "Warm"] as ChatTone[]).map((t) => (
                  <button
                    ...
                    onClick={() => setAiTone(t)}
                    style={{
                      flex: 1, fontSize: '10px', fontWeight: '700', padding: '6px 0',
                      borderRadius: '8px', transition: 'all 0.2s',
                      background: aiTone === t ? 'white' : 'transparent',
                      color: aiTone === t ? '#1E3A8A' : '#64748b', border: 'none', cursor: 'pointer',
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '4px' }}>
                {[
                  { intent: "icebreaker" as UIIntent, label: "🧊 Icebreaker", color: '#1e293b', hover: '#dbeafe' },
                  { intent: "reply" as UIIntent, label: "↩️ Polite Reply", color: '#1e293b', hover: '#dbeafe' },
                  { intent: "decline" as UIIntent, label: "🚫 Say No Nicely", color: '#ef4444', hover: '#fee2e2' },
                ].map(({ intent, label, color, hover }) => (
                  <button
                    key={intent}
                    onClick={() => handleAiAssist(intent)}
                    disabled={aiLoading}
                    style={{
                      textAlign: 'left', padding: '12px', background: 'transparent',
                      border: 'none', borderRadius: '12px', fontSize: '12px', fontWeight: '500',
                      color, cursor: aiLoading ? 'not-allowed' : 'pointer',
                      transition: 'background 0.2s', display: 'flex', alignItems: 'center',
                      gap: '8px', opacity: aiLoading ? 0.5 : 1
                    }}
                    onMouseEnter={(e) => !aiLoading && (e.currentTarget.style.background = hover)}
                    onMouseLeave={(e) => e.currentTarget.style.background = 'transparent'}
                  >
                    {label}
                  </button>
          ...
          <div style={{ display: 'flex', gap: '8px', alignItems: 'center', maxWidth: '640px', margin: '0 auto' }}>
            {/* AI Toggle — hidden when partner deleted */}
            {!partnerDeleted && (
              <button
                ...
                style={{
                  ...
                  border: '1px solid rgba(30, 58, 138, 0.2)', cursor: 'pointer',
                  transition: 'all 0.2s', boxShadow: '0 2px 8px rgba(30, 58, 138, 0.1)'
                }}
            ...
            <input
              ...
              placeholder={
                partnerDeleted ? "This conversation is archived" :
                isLimitReached ? "Unlock to continue..." : "Type a message..."
              }
              disabled={isLimitReached || partnerDeleted}
              style={{
                flex: 1, background: 'rgba(248, 250, 252, 0.8)',
                border: '1.5px solid rgba(226, 232, 240, 0.5)',
                borderRadius: '24px', padding: '12px 20px', fontSize: '14px',
                outline: 'none', transition: 'all 0.2s', color: '#1e3a8a', fontFamily: 'inherit'
              }}
            ...
            {!partnerDeleted && (
              <button
                ...
                style={{
                  ...
                  display: 'flex', alignItems: 'center', justifyContent: 'center',
                  color: 'white',
                  background: isLimitReached ? '#f59e0b' : 'linear-gradient(135deg, #1e3a8a 0%, #2563eb 100%)',
                  border: 'none', cursor: 'pointer',
                  boxShadow: isLimitReached ? '0 4px 12px rgba(245, 158, 11, 0.3)' : '0 4px 12px rgba(30, 58, 138, 0.3)',
                  transition: 'all 0.2s'
                }}
      ...
      {showContactModal && (
        <div style={{
          ...
          WebkitBackdropFilter: 'blur(4px)', padding: '16px'
        }}>
          <div style={{
            background: 'rgba(255, 255, 255, 0.95)', backdropFilter: 'blur(20px)',
            WebkitBackdropFilter: 'blur(20px)', width: '100%', maxWidth: '400px',
            borderRadius: '24px', padding: '32px 24px',
            boxShadow: '0 20px 60px rgba(31, 41, 55, 0.3)', textAlign: 'center'
          }}>
            <div style={{
            ...
            <h2 style={{ fontSize: '20px', fontWeight: '700', color: '#1e3a8a', marginBottom: '8px', fontFamily: 'Georgia, serif' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
              <button
                ...
                style={{
                  width: '100%', background: 'linear-gradient(135deg, #1e3a8a 0%, #2563eb 100%)',
                  color: 'white', fontWeight: '700', padding: '16px', borderRadius: '12px',
                  display: 'flex', justifyContent: 'center', alignItems: 'center', gap: '8px',
                  boxShadow: '0 4px 16px rgba(30, 58, 138, 0.3)', border: 'none',
                  cursor: sendingCard ? 'not-allowed' : 'pointer', transition: 'all 0.2s',
                  opacity: sendingCard ? 0.6 : 1, fontFamily: 'inherit'
                }}
              >
                {sendingCard ? <Loader2 style={{ animation: 'spin 1s linear infinite' }} /> : <><Contact size={18} /> Share Card (199 Coins)</>}
              </button>
              <button
                onClick={() => setShowContactModal(false)}
                style={{
                  width: '100%', color: '#64748b', fontWeight: '500', padding: '8px',
      ...
      {showLimitModal && (
        <div style={{
          ...
          <div style={{
            ...
            <div style={{
              width: '64px', height: '64px', background: 'rgba(30, 58, 138, 0.1)',
            ...
            <p style={{ fontSize: '14px', color: '#64748b', marginBottom: '24px', padding: '0 16px', lineHeight: '1.5' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
              ...
              <button
                ...
                style={{
                  width: '100%', color: '#64748b', fontWeight: '500',
