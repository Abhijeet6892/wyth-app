...
// ✅ FIX 1: Use @/ alias for stability (Matches your Backup Code)
...
// ✅ FIX 2: Use @/ alias for components
import AvatarUpload from '@/components/AvatarUpload'

// ✅ FIX 3: Use @/ alias for actions (and handle potential missing type export)
import { generateBioAction } from '@/app/actions/generateBio'

// --- TYPES ---
// Defined locally to prevent import crashes
type BioTone = 'Chill' | 'Witty' | 'Romantic'
...
const KNOWN_CITIES = [
  ...
  "Jaipur","Udaipur","Jodhpur","Ajmer","Chandigarh","Mohali","Panchkula",
  "Lucknow","Kanpur","Agra","Varanasi","Prayagraj","Noida Extension",
  "Indore","Bhopal","Jabalpur","Gwalior","Bhubaneswar","Cuttack",
  "Patna","Gaya","Muzaffarpur","Ranchi","Jamshedpur","Dhanbad",
  "Raipur","Bilaspur","Nagpur","Nashik","Aurangabad","Solapur","Kolhapur",
  "Thane","Navi Mumbai","Kalyan","Dombivli","Amritsar","Ludhiana","Jalandhar","Patiala",
  "Dehradun","Haridwar","Roorkee","Shimla","Solan","Srinagar","Jammu",
  "Guwahati","Silchar","Dibrugarh","Shillong","Imphal","Aizawl","Kohima","Dimapur",
  "Agartala","Gangtok","Itanagar","Panaji","Margao","Vasco da Gama",
  ...
  "Warangal","Nizamabad","Karimnagar","Mangaluru","Udupi","Mysuru","Hubballi",
  "Belagavi","Davangere","Shivamogga","Hisar","Rohtak","Panipat","Sonipat","Karnal",
  "Rewari","Bhiwani","Alwar","Bharatpur","Siliguri","Asansol","Durgapur",
  "Howrah","Hooghly","Kharagpur","Port Blair","Leh","Kargil",
  "New York","San Francisco","San Jose","Los Angeles","San Diego",
  "Seattle","Redmond","Bellevue","Chicago","Austin","Dallas","Houston",
  "Boston","Cambridge","Washington DC","Arlington","Reston","Atlanta","Miami",
  "Orlando","San Mateo","Mountain View","Sunnyvale","Palo Alto","Cupertino",
  "Fremont","Milpitas","Newark","Jersey City","Edison","Princeton","Raleigh",
  "Durham","Chapel Hill","Toronto","Mississauga","Brampton","Scarborough",
  "Vancouver","Burnaby","Surrey","Richmond","Calgary","Edmonton","Montreal",
  "Laval","Ottawa","Waterloo","Kitchener","London","Greater London","Canary Wharf",
  "Manchester","Birmingham","Leeds","Sheffield","Reading","Slough","Wembley",
  "Milton Keynes","Oxford","Cambridge","Leicester","Nottingham","Dubai","Abu Dhabi",
  "Sharjah","Ajman","Al Ain","Ras Al Khaimah","Fujairah","Sydney","Melbourne",
  "Brisbane","Perth","Adelaide","Canberra","Parramatta","Auckland","Wellington",
  "Christchurch","Berlin","Munich","Frankfurt","Hamburg","Stuttgart","Dusseldorf",
  "Paris","La Defense","Lyon","Marseille","Nice","Amsterdam","Rotterdam","The Hague",
  "Utrecht","Singapore","Tokyo","Yokohama","Osaka","Kyoto","Nagoya","Hong Kong",
  "Shenzhen","Shanghai","Beijing","Guangzhou","Dublin","Cork","Galway","Milan",
  "Rome","Turin","Florence","Barcelona","Madrid","Valencia","Zurich","Geneva",
  "Basel","Stockholm","Gothenburg","Oslo","Copenhagen","Brussels","Lisbon","Porto",
  "Johannesburg","Cape Town","Pretoria","Doha","Riyadh","Jeddah","Kuala Lumpur",
  "Petaling Jaya","Jakarta","Bali","Bangkok","Seoul","Sao Paulo","Rio de Janeiro",
  "Mexico City","Moscow","Saint Petersburg"
]
...
export default function OnboardingEngine() {
  ...
  return (
    // ✅ FIX 4: Use Global CSS 'bg-brand-gradient'
    <div className="min-h-screen bg-brand-gradient flex flex-col items-center py-6">
      {/* ✅ FIX 5: Use Global CSS 'glass-panel' */}
      <div className="w-full max-w-md glass-panel min-h-[80vh] rounded-3xl shadow-xl overflow-hidden flex flex-col relative">
        
        {/* HEADER */}
        {step < TOTAL_STEPS && (
            <div className="px-6 py-4 border-b border-slate-100/50 flex items-center justify-between bg-white/40 backdrop-blur-sm sticky top-0 z-20">
                {step > 1 ? <button onClick={back} className="p-2 -ml-2 hover:bg-slate-50/50 rounded-full"><ArrowLeft size={20}/></button> : <div className="w-8"></div>}
                <div className="flex gap-1">{[...Array(TOTAL_STEPS)].map((_, i) => (<div key={i} className={`h-1.5 w-6 rounded-full transition-all duration-300 ${i + 1 <= step ? 'bg-slate-900' : 'bg-slate-200/50'}`} />))}</div>
        ...
        {/* FOOTER */}
        <div className="p-6 border-t border-slate-100/50 bg-white/40 sticky bottom-0 z-20 backdrop-blur-md">
            <button onClick={saveAndNext} disabled={loading} className="w-full bg-slate-900 text-white font-bold h-14 rounded-2xl flex items-center justify-center gap-2 hover:scale-[1.02] active:scale-[0.98] transition-all disabled:opacity-70 shadow-lg">
...
function StageIntent({ data, update }: StageProps) {
  ...
  return (
    <div className="animate-in slide-in-from-right duration-300 space-y-6">
      ...
      <div>
          ...
          <input value={data.full_name} onChange={e => update('full_name', e.target.value)} className="w-full p-4 rounded-xl bg-white/60 border border-slate-100 mt-2 font-medium focus:ring-2 focus:ring-slate-900 outline-none transition" placeholder="e.g. Arjun Kapoor" />
      </div>
      <div>
          <label className="text-xs font-bold text-slate-400 uppercase">Phone Number</label>
          <div className="relative">
             <Phone size={18} className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-400"/>
             <input type="tel" value={data.phone} onChange={e => update('phone', e.target.value.replace(/[^\d+]/g, ''))} className="w-full p-4 pl-12 rounded-xl bg-white/60 border border-slate-100 mt-2 font-medium focus:ring-2 focus:ring-slate-900 outline-none transition" placeholder="+91 9876543210" />
          </div>
      </div>
      <div ref={wrapperRef} className="relative">
          ...
          <div className="relative mt-2">
            ...
            <input value={cityQuery} onChange={e => { setCityQuery(e.target.value); setShowCityList(true); }} onFocus={() => setShowCityList(true)} className="w-full p-4 pl-12 rounded-xl bg-white/60 border border-slate-100 font-medium focus:ring-2 focus:ring-slate-900 outline-none transition" placeholder="Search city..." />
          </div>
          {showCityList && (
              <div className="absolute z-50 w-full mt-2 bg-white rounded-xl shadow-xl border border-slate-100 max-h-60 overflow-y-auto">
                  {filteredCities.map(city => (
                      <div key={city} onClick={() => selectCity(city, 'known')} className="p-3 hover:bg-slate-50 cursor-pointer text-sm font-medium border-b border-slate-50 last:border-none">{city}</div>
                  ))}
                  {cityQuery && !KNOWN_CITIES.includes(cityQuery) && (
                      <div onClick={() => selectCity(cityQuery, 'other')} className="p-3 hover:bg-blue-50 cursor-pointer text-sm font-bold text-blue-600 border-t border-slate-100">Use "{cityQuery}" (Other)</div>
                  )}
              </div>
          )}
      </div>
      <div>
          ...
          <div className="flex gap-2 mt-2">{['Male', 'Female', 'Other'].map(g => (<button key={g} onClick={() => update('gender', g)} className={`flex-1 py-3 rounded-xl font-medium transition ${data.gender === g ? 'bg-slate-900 text-white shadow-lg' : 'bg-white/60 border border-slate-100 text-slate-500 hover:bg-white'}`}>{g}</button>))}</div>
      </div>
    </div>
  )
}

function StagePhotos({ data, update }: StageProps) {
    return (
      <div className="animate-in slide-in-from-right duration-300 space-y-6">
...
function StageCareer({ data, update }: StageProps) {
  return (
    <div className="animate-in slide-in-from-right duration-300 space-y-6">
      ...
      <div className="bg-blue-50/50 p-4 rounded-2xl border border-blue-100 mb-6 backdrop-blur-sm">
      ...
      <input value={data.jobTitle} onChange={e => update('jobTitle', e.target.value)} placeholder="Job Title" className="w-full p-4 rounded-xl bg-white/60 border border-slate-100 font-medium focus:ring-2 focus:ring-slate-900 outline-none transition" />
      <input value={data.company} onChange={e => update('company', e.target.value)} placeholder="Company Name" className="w-full p-4 rounded-xl bg-white/60 border border-slate-100 font-medium focus:ring-2 focus:ring-slate-900 outline-none transition" />
      <input value={data.industry} onChange={e => update('industry', e.target.value)} placeholder="Industry" className="w-full p-4 rounded-xl bg-white/60 border border-slate-100 font-medium focus:ring-2 focus:ring-slate-900 outline-none transition" />
    </div>
  )
}

function StageLifestyle({ data, update }: StageProps) {
...
function StagePrivateSignals({ data, updateSignal }: { data: ProfileData, updateSignal: (field: keyof ProfileSignals, value: any) => void }) {
  ...
  return (
    <div className="animate-in slide-in-from-right duration-300 space-y-8">
      ...
      <div className="bg-slate-50/50 p-4 rounded-xl border border-slate-100 flex gap-3 items-center mb-2 backdrop-blur-sm"><div className="p-2 bg-white rounded-full text-slate-400"><Lock size={16}/></div><p className="text-xs text-slate-500">This data is <b>not</b> shown on your public profile.</p></div>
      ...
      <div className="space-y-4 pt-4 border-t border-slate-100">
        ...
        <select value={signals.religionSignal || ''} onChange={e => updateSignal('religionSignal', e.target.value)} className="w-full p-4 rounded-xl bg-white/60 border border-slate-100 outline-none focus:ring-2 focus:ring-slate-900 transition"><option value="">Prefer not to say</option><option value="Hindu">Hindu</option><option value="Muslim">Muslim</option><option value="Christian">Christian</option><option value="Sikh">Sikh</option></select>
        <label className="font-bold text-slate-900 block mt-4">Family Setup</label>
        <div className="flex gap-2">{['Nuclear', 'Joint', 'Flexible'].map(t => (<button key={t} onClick={() => updateSignal('familyTypeSignal', t)} className={`flex-1 py-3 rounded-xl font-medium border ${signals.familyTypeSignal === t ? 'bg-slate-900 text-white border-slate-900 shadow-md' : 'border-slate-200 text-slate-500 bg-white/60'}`}>{t}</button>))}</div>
      </div>
    </div>
  )
}

function StageAIBio({ data, update }: StageProps) {
  ...
  return (
    <div className="animate-in slide-in-from-right duration-300 relative h-full flex flex-col">
      ...
      <div className="relative flex-1">
        <textarea value={data.bio} onChange={e => update('bio', e.target.value)} placeholder="I enjoy travelling..." className="w-full h-48 p-4 rounded-2xl bg-white/60 border border-slate-100 resize-none focus:ring-2 focus:ring-slate-900 outline-none text-lg leading-relaxed"/>
...
function StageWelcome({ data }: { data: ProfileData }) {
    return (
        <div className="animate-in slide-in-from-right duration-500 flex flex-col items-center text-center pt-8">
            <div className="w-24 h-24 bg-green-50 rounded-full flex items-center justify-center mb-6 animate-bounce shadow-lg">
            ...
            <div className="w-full space-y-4 text-left">
                <div className="flex items-center gap-4 p-4 bg-rose-50/50 rounded-2xl border border-rose-100">
                    <div className="p-2 bg-white rounded-full shadow-sm"><Zap size={20} className="text-rose-600 fill-rose-600"/></div>
                    <div>
                ...
                <div className="flex items-center gap-4 p-4 bg-amber-50/50 rounded-2xl border border-amber-100">
                    <div className="p-2 bg-white rounded-full shadow-sm"><DollarSign size={20} className="text-amber-600"/></div>
                    <div>
                ...
                <div className="flex items-center gap-4 p-4 bg-blue-50/50 rounded-2xl border border-blue-100">
                    <div className="p-2 bg-white rounded-full shadow-sm"><Shield size={20} className="text-blue-600"/></div>
                    <div>
